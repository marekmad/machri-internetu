...
<body id="questionpage" onload="loadLastTime()">
	<a class="accessibility" href="BigQuizServlet?action=question">Zur
		Frage springen</a>
	<header role="banner" aria-labelledby="mainheading">
		...
		<input id="questionid" type="hidden"
			value=<%=game.getAcctuallQuestion().getId()%> accesskey="s" />
	</header>
	...
	<section role="main">
		<section id="roundinfo" aria-labelledby="roundinfoheading">
			...
			<div id="player1info">
				...
				<ul class="playerroundsummary">
					<li><span class="accessibility">Frage 1:</span><span
						id="player1answer0"
						class=<%=game.getPlayer1().getQuestionState(0) %>> <%=game.getPlayer1().getQuestionState(0) %></span></li>
					<li><span class="accessibility">Frage 2:</span><span
						id="player1answer1"
						class=<%=game.getPlayer1().getQuestionState(1) %>> <%=game.getPlayer1().getQuestionState(1) %></span></li>
					<li><span class="accessibility">Frage 3:</span><span
						id="player1answer2"
						class=<%=game.getPlayer1().getQuestionState(2) %>> <%=game.getPlayer1().getQuestionState(2) %></span></li>
			...
			<div id="player2info">
				...
				<ul class="playerroundsummary">
					<li><span class="accessibility">Frage 1:</span><span
						id="player2answer0"
						class=<%=game.getPlayer2().getQuestionState(0) %>><%=game.getPlayer2().getQuestionState(0) %></span></li>
					<li><span class="accessibility">Frage 2:</span><span
						id="player2answer1"
						class=<%=game.getPlayer2().getQuestionState(1) %>><%=game.getPlayer2().getQuestionState(1) %></span></li>
					<li><span class="accessibility">Frage 3:</span><span
						id="player2answer2"
						class=<%=game.getPlayer2().getQuestionState(2) %>><%=game.getPlayer2().getQuestionState(2) %></span></li>

				</ul>
			</div>
			<div id="currentcategory">
				<span class="accessibility">Kategorie:</span>
				<%=game.getAcctuallQuestion().getCategory().getName() %></div>
		</section>

		<!-- Question -->
		<section id="question" aria-labelledby="questionheading">

			<form id="questionform" action="BigQuizServlet?action=submitAnswer"
				method="post">
				...
				<ul id="answers">
					...
					<% for (int i = 0; i < 4; i ++) { %>
					<li><input id=<%=choices.get(i).getId()%> type="checkbox"
						name="opts" value="<%=choices.get(i).getId()%>" /><label
						for=<%=choices.get(i).getId()%>><%= choices.get(i).getText() %>
					</label></li>
				...
				<input id="timeleftvalue" type="hidden" value="100" /> <input
					id="next" type="submit" value="weiter" accesskey="s" />
			</form>
		</section>
		...
		<section id="timer" aria-labelledby="timerheading">
			...
			<p>
				...
				<time id="timeleft"><%=game.getAcctuallQuestion().getFormatedMaxTime()%></time>
			</p>
		...
		<section id="lastgame">
			<p>
				Letztes Spiel: <label id="lastTime"></label>
			</p>
		</section>
	</section>
	...
	<script type="text/javascript">
		...
		function loadLastTime() {
			if (typeof (Storage) !== "undefined") {
				...
				var storedValue = localStorage.getItem('lastTime');
				if (storedValue) {
					document.getElementById('lastTime').innerHTML = storedValue;

				} else {
					document.getElementById('lastTime').innerHTML = "Nie";
				}
